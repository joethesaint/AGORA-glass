    ...
    <div className="min-h-screen bg-[#0B0E14] flex items-center justify-center p-6 overflow-hidden relative text-white">
        ...
        {step === 0 && (
          ...
          >
            <div className="text-center space-y-4 group">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-accent/10 border border-accent/20 text-accent text-[10px] font-bold uppercase tracking-widest mb-4 hover:bg-accent/20 transition-all cursor-default">
              ...
              <p className="text-xl text-muted max-w-2xl mx-auto leading-relaxed font-mono font-bold transition-all duration-500 group-hover:text-white/80">
              ...
              {[
                {
                  icon: Shield,
                  title: "Safety First",
                  desc: "Maintains a 12% margin ratio safety band automatically via Circle's Developer-Controlled Wallets."
                },
                {
                  icon: LayoutGrid,
                ...
                },
                {
                  ...
                  desc: "Optimized for speed using Circle's Developer-Controlled Wallets and Arc's fast finality."
                }
              ].map((feature, i) => (
                <div key={i} className="agora-card p-6 space-y-3 border-white/5 hover:border-accent/40 transition-all group cursor-default">
                  ...
                  <h3 className="text-lg font-black tracking-tight text-white group-hover:text-accent transition-colors uppercase">{feature.title}</h3>
                  <p className="text-sm font-mono text-muted leading-relaxed group-hover:text-white/70 transition-colors">{feature.desc}</p>
                </div>
              ))}
            </div>
            ...
            <div className="flex justify-center pt-8">
              <button 
                onClick={handleNext}
                className="group relative flex items-center gap-3 px-12 py-6 bg-accent text-white rounded-2xl font-black text-2xl transition-all hover:scale-105 active:scale-95 shadow-[0_0_30px_rgba(0,163,255,0.4)] uppercase tracking-tighter"
              >
          ...
          >
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-accent transition-colors font-mono"
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-muted uppercase tracking-widest">
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    ...
                    >
                      <div className="flex items-center justify-between w-full mb-1">
                        ...
                        {(connectionType === 'web2' || address?.includes('Circle') || (isMock && !connectionType && address)) && <CheckCircle2 className="w-4 h-4 text-accent" />}
                      </div>
                    ...
                    >
                      <div className="flex items-center justify-between w-full mb-1">
                        ...
                        {(connectionType === 'web3' || address?.includes('Web3')) && <CheckCircle2 className="w-4 h-4 text-accent" />}
                      </div>
                ...
                <div className="space-y-6">
                  <div className="p-4 rounded-xl bg-white/5 border border-white/10 space-y-4">
                    <div className="flex justify-between items-center text-xs font-mono">
                      <span className="text-muted uppercase font-bold">Target Account</span>
                      <span className="text-white">{isMock ? 'MOCK_SENTINEL_TARGET' : hlAccount.slice(0, 8) + '...'}</span>
                    </div>
                    <div className="flex justify-between items-center text-xs font-mono">
                      <span className="text-muted uppercase font-bold">Vault Allocation</span>
                      <span className="text-accent font-bold">${vaultAmount} USDC</span>
                    </div>
                    <div className="flex justify-between items-center text-xs font-mono">
                      <span className="text-muted uppercase font-bold">Deployment Mode</span>
            ...
            <div className="flex items-center gap-3 pt-4">
              ...
              <button 
                ...
                className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-accent disabled:opacity-50 disabled:grayscale text-white rounded-xl font-bold text-sm hover:bg-[#008BDB] transition-all shadow-[0_0_20px_rgba(0,163,255,0.3)] uppercase tracking-widest"
              >
      ...
      <footer className="absolute bottom-8 left-0 w-full pointer-events-none">
        <div className="container mx-auto px-6 text-center text-[10px] text-[#484848]">
          <p className="font-bold text-muted mb-1 tracking-widest uppercase">GLASS</p>
