...
export function PositionHealthCard() {
  // Narrow selector: this component only reads livePositions, so it should
  // only re-render when that slice changes — not on every other field
  // update the store receives from the WS stream (unlike a whole-store
  // destructure, which re-subscribes to every field).
  const livePositions = useAnalyticsStore((state) => state.livePositions);
  ...
  const getMarginColor = () => {
    if (isCritical) return 'text-neg';
    if (isWarning) return 'text-warn';
    return 'text-pos';
  };

  const getMarginBg = () => {
    if (isCritical) return 'bg-neg';
    if (isWarning) return 'bg-warn';
    return 'bg-pos';
  };
  ...
  return (
    <motion.div
      ...
      className={`agora-card relative overflow-visible border ${isCritical ? 'border-neg/30 shadow-[0_0_30px_rgba(255,59,59,0.1)]' : 'border-white/5'}`}
    >
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          <div className={`p-2 rounded-lg ${isCritical ? 'bg-neg/10' : 'bg-pos/10'}`}>
            <Activity className={`w-4 h-4 ${isCritical ? 'text-neg' : 'text-pos'}`} />
          </div>
        ...
        <div className="relative">
          ...
          >
            ...
            <ChevronDown className={`w-3 h-3 text-muted transition-transform ${showDropdown ? 'rotate-180' : ''}`} />
          </button>
                ...
                >
                  {positions.map((p) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2.5 text-[10px] font-mono transition-colors ${selectedId === p.id
      ...
      <div className="space-y-8">
        ...
        <div className="space-y-3">
          <div className="flex items-end justify-between">
            <div>
              <p className="text-[10px] font-bold text-muted uppercase tracking-[0.2em] mb-1">Margin Ratio</p>
          ...
          <div className="relative h-2 w-full bg-white/5 rounded-full overflow-hidden">
            ...
            />
            {/* Threshold Markers */}
            <div className="absolute top-0 bottom-0 w-0.5 bg-neg opacity-50 z-10" style={{ left: `${CRITICAL_THRESHOLD * 100}%` }} />
            <div className="absolute top-0 bottom-0 w-0.5 bg-warn opacity-30 z-10" style={{ left: `${WARNING_THRESHOLD * 100}%` }} />
          </div>
        ...
        <div className="space-y-3 pt-4 border-t border-white/5">
          <div className="flex items-end justify-between">
            <div>
              <p className="text-[10px] font-bold text-muted uppercase tracking-[0.2em] mb-1">Portfolio Leverage</p>
              <div className={`text-3xl font-black font-mono ${leverage > 5 ? 'text-neg' : 'text-white'}`}>
          ...
          <div className="relative h-2 w-full bg-white/5 rounded-full overflow-hidden">
            <motion.div
              ...
              transition={{ type: "spring", stiffness: 100, damping: 20 }}
              className={`h-full ${leverage > 5 ? 'bg-neg' : 'bg-accent'} shadow-[0_0_15px_rgba(0,0,0,0.5)]`}
            />
            <div className="absolute top-0 bottom-0 w-0.5 bg-neg opacity-50 z-10" style={{ left: '50%' }} />
          </div>
        ...
        <div className="flex justify-center pt-2">
          <div className={`flex items-center gap-2 px-6 py-2 rounded-xl border font-black text-xs tracking-tighter transition-all ${isCritical
              ? 'bg-neg/10 border-neg/20 text-neg shadow-[0_0_20px_rgba(255,59,59,0.1)]'
              : isWarning
        ...
        <div className="grid grid-cols-3 gap-2 pt-4 border-t border-white/5">
          <div className="text-left">
            <p className="text-[9px] font-bold text-muted uppercase tracking-widest mb-1">Unrealized PnL</p>
            <p className={`text-sm font-black font-mono ${activePosition.unrealizedPnL >= 0 ? 'text-pos' : 'text-neg'}`}>
              ${activePosition.unrealizedPnL?.toFixed(2) || '0.00'}
            </p>
          </div>
          <div className="text-center border-x border-white/5">
            <p className="text-[9px] font-bold text-muted uppercase tracking-widest mb-1">Collateral</p>
            <p className="text-sm font-bold text-white font-mono">
              ${activePosition.collateral?.toLocaleString() || '0'}
            </p>
          </div>
          <div className="text-right">
            <p className="text-[9px] font-bold text-muted uppercase tracking-widest mb-1">Position Size</p>
            <p className="text-sm font-bold text-white font-mono">
