...
async def test_rescue_dispatcher_success(mocker):
    ...
    assert results[0].tx_hash == "0xTX_RESCUE"
    assert results[0].amount == 100.0
    mock_pin.assert_called_once_with("0x123")
    mock_rescue.assert_called_once_with(amount=100.0, destination_address="0xRECIPIENT", reason_hash="0x123")
